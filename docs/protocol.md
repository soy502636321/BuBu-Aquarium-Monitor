...
# 3. 数据帧格式
采集板对主机通信采用数据帧格式：
考虑到后期的拓展，传感器采集的数据不一定是固定数量的，数据长度采用动态长度
动态长度数据建议采用 TLV（Type-Length-Value）格式

```
+--------+--------+--------+--------+--------+-------------+--------+
| HEAD   | VER    | TYPE   | LEN    | COUNT  | TLV DATA    | CRC16  |
+--------+--------+--------+--------+-------------+--------+--------+
| 2Byte  | 1Byte  | 1Byte  | 2Byte  |1Byte   | N Byte      | 2Byte  |
+--------+--------+--------+--------+--------+-------------+--------+
```
...
| TYPE | 1Byte | 消息类型 | 02 |
| LEN | 2Byte | DATA长度 | 20 |
| COUNT | 1Byte | 里面有多少个参数 | 2 |
| TLV DATA | N Byte | 数据内容 | 属性数据 |
