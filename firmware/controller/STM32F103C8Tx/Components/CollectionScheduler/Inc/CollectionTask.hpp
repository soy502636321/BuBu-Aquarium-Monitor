...
struct CollectionTask {
    ...
    uint32_t interval_ms;                // 采集间隔 (毫秒)
    uint32_t last_run_ms;                // 上次运行时间
    std::function<void()> callback;      // 采集回调函数
    bool enabled;                        // 是否启用

    // 构造函数
    CollectionTask(const char* n, uint32_t interval, std::function<void()> cb)
        : name(n), interval_ms(interval), last_run_ms(0), callback(cb), enabled(true) {}

    CollectionTask(const char* n, uint32_t interval, std::function<void()> cb, bool en)
        : name(n), interval_ms(interval), last_run_ms(0), callback(cb), enabled(en) {}
};
}
